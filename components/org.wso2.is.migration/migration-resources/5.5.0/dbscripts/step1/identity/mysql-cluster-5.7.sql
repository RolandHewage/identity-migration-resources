...
BEGIN
    IF ((SELECT COUNT(*) AS index_exists
         ...
         WHERE TABLE_SCHEMA = DATABASE()
           ...
           AND index_name = 'IDX_IOAT_AT') > 0) THEN
        SET @s = CONCAT('DROP INDEX ', 'IDX_IOAT_AT', ' ON ', 'IDN_OAUTH2_ACCESS_TOKEN'); PREPARE stmt FROM @s;
        EXECUTE stmt;
    END IF;
END;
...
BEGIN
    IF ((SELECT COUNT(*) AS index_exists
         ...
         WHERE TABLE_SCHEMA = DATABASE()
           ...
           AND index_name = 'IDX_AUTHORIZATION_CODE') > 0) THEN
        SET @s = CONCAT('DROP INDEX ', 'IDX_AUTHORIZATION_CODE', ' ON ', 'IDN_OAUTH2_AUTHORIZATION_CODE');
...
ALTER TABLE IDN_OAUTH2_AUTHORIZATION_CODE
    modify AUTHORIZATION_CODE VARCHAR(2048);
ALTER TABLE IDN_OAUTH_CONSUMER_APPS
    modify CONSUMER_SECRET VARCHAR(2048);
