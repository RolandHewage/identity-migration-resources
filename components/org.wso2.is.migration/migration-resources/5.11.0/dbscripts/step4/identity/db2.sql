...
BEGIN
  ...
  select CONSTNAME into const_name from SYSCAT.TABCONST WHERE TABNAME='IDN_OIDC_PROPERTY' AND TYPE = 'F';
  SET STMT = 'ALTER TABLE IDN_OIDC_PROPERTY DROP FOREIGN KEY ' ||  const_name;
...
FROM ( VALUES (1) )
WHERE EXISTS (SELECT * FROM IDN_OAUTH_CONSUMER_APPS WHERE CONSUMER_KEY= 'MY_ACCOUNT')
/
