...
CREATE PROCEDURE skip_constraint_if_exists(constraintName varchar(64), tableName varchar(64), tableColumns varchar(255)) BEGIN BEGIN DECLARE CONTINUE HANDLER FOR SQLEXCEPTION BEGIN END; SET @s = CONCAT('ALTER TABLE ', tableName, ' ADD CONSTRAINT ', constraintName,' UNIQUE' tableColumns); PREPARE stmt FROM @s; EXECUTE stmt; END;END;
