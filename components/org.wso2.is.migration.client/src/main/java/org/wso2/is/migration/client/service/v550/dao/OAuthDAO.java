...
import org.wso2.is.migration.client.service.v550.bean.ClientSecretInfo;
import org.wso2.is.migration.client.service.v550.SQLConstants;
...
public class OAuthDAO {
    ...
    public boolean isConsumerSecretHashColumnAvailable(Connection connection) throws MigrationClientException {
        ...
        try {
            if (connection.getMetaData().getDriverName().contains("MySQL") || connection.getMetaData().getDriverName()
                    .contains("H2")) {
                sql = SQLConstants.RETRIEVE_CONSUMER_APPS_TABLE_MYSQL;
            } else if (connection.getMetaData().getDatabaseProductName().contains("DB2")) {
                sql = SQLConstants.RETRIEVE_CONSUMER_APPS_TABLE_DB2SQL;
            } else if (connection.getMetaData().getDriverName().contains("MS SQL") || connection.getMetaData()
                    .getDriverName().contains("Microsoft")) {
                sql = SQLConstants.RETRIEVE_CONSUMER_APPS_TABLE_MSSQL;
            } else if (connection.getMetaData().getDriverName().contains("PostgreSQL")) {
                sql = SQLConstants.RETRIEVE_CONSUMER_APPS_TABLE_MYSQL;
            } else if (connection.getMetaData().getDriverName().contains("Informix")) {
                // Driver name = "IBM Informix JDBC Driver for IBM Informix Dynamic Server"
                sql = SQLConstants.RETRIEVE_CONSUMER_APPS_TABLE_INFORMIX;
            } else {
                sql = SQLConstants.RETRIEVE_CONSUMER_APPS_TABLE_ORACLE;
            }
        } catch (Exception e) {
    ...
    public void deleteConsumerSecretHashColumn(Connection connection) throws SQLException {
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(SQLConstants.DELETE_CONSUMER_SECRET_HASH_COLUMN)) {
    ...
    public List<ClientSecretInfo> getAllClientSecrets(Connection connection) throws SQLException {
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(SQLConstants.RETRIEVE_ALL_CONSUMER_SECRETS);
    ...
    public void updateNewClientSecrets(List<ClientSecretInfo> updatedClientSecretList, Connection connection)
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(SQLConstants.UPDATE_CONSUMER_SECRET)) {
